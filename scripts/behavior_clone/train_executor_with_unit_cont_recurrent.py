...
from dataset_recurrent import BehaviorCloneDataset, merge_max_units
...
def train(model, device, optimizer, grad_clip, data_loader, epoch, stat):
    ...
    for batch_idx, batch in enumerate(data_loader):
        ...
        optimizer.zero_grad()
        loss, all_losses = model.compute_traj_loss(batch, mean=False)
...
def main():
    ...
    model = Executor(args['executor'], options.num_resource_bin, recurrent=True).to(device)
